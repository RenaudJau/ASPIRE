...
#'
#' @return \item{values}{value or vector of values given by the argument values}
#' @return \item{utility}{value or vector of values given by the argument utility}
#' @return \item{ref}{value or vector of values given by the argument ref}
#' @return \item{mean_values}{mean value of the variable}
#' @return \item{inf_values}{mean value of the variable minus error (useful for graphical purpose)}
#' @return \item{sup_values}{mean value of the variable plus error (useful for graphical purpose)}
#' @return \item{mean_ref}{mean value of the reference}
#' @return \item{inf_ref}{mean value of the reference minus error (useful for graphical purpose)}
#' @return \item{sup_ref}{mean value of the reference plus error (useful for graphical purpose)}
#' @return \item{transf_values}{variable scores, i.e. transformed values (cf function \code{\link{Transf_ASPIRE}})}
#' @return \item{mean_transf_values}{mean variable score}
#' @return \item{mean_transf_values_limited}{mean variable score, but limited to 1 (useful for radarplot drawing)}
#' @return \item{inf_transf_values}{mean score value of the variable minus error (useful for graphical purpose)}
#' @return \item{inf_transf_values_limited}{mean value of the variable minus error, but limited to 1  (useful for radarplot drawing)}
#' @return \item{sup_transf_values}{mean score value of the variable plus error (useful for graphical purpose)}
#' @return \item{sup_transf_values_limited}{mean score value of the variable plus error, but limited to 1 (useful for radarplot drawing)}
#' @return \item{transf_ref}{mean score for the reference}
#' @return \item{sup_transf_ref}{mean score for the reference plus error (useful for graphical purpose)}
#' @return \item{inf_transf_ref}{mean score for the reference minus error (useful for graphical purpose)}
...
Variable<-function(values,ref,utility = "continuous",ALPHA=NULL,XVAL=NULL,YSCO=NULL,NEG=FALSE)
{
  ...
  Calculated_Variable<-list(ref=ref[is.na(ref)==F],mean_ref,error_ref,inf_ref,sup_ref,nb_ref,
                            ...
                            transf_values,mean_transf_value,mean_transf_value_limited,error_transf_value,inf_transf_value,inf_transf_value_limited,sup_transf_value,sup_transf_value_limited)
...
Objective<-function(var_names,var_weight)
{
  ...
  Output<-list(Objective_val,Summary_Var_table,Var_table)
  names(Output)<-c("Objective_val","Summary_Var_table","Var_table")
  return(Output)
}
```
...
Project<-function(obj_name, stakeholders, obj_weight)
{
  ...
  Output<-list(Summary_Proj_Score,Proj_table,Summary_All,Var_table)
  names(Output)<-c("Summary_Proj_Score","Proj_table","Summary_All","Var_table")
  return(Output)
}
```
                            ...
                            las.x = 1,cex.x = 1)
{
  ...
  for (i in 1:(nplot-2))
  {
    Obj.barplot(obj = get(as.vector(proj$Summary_All$Objectives$name_objective)[i]), 
...
Proj.all.radar.plot<-function(proj,transp=0.5,col=NULL,AllInOne=TRUE,limited=TRUE,main="",plot.ref=TRUE,cex.lab=1)
{
  ...
  if(AllInOne==TRUE){
    ...
    for (i in 1:nplot)
    {
      ...
      Obj.radar.plot(obj = get(as.vector(proj$Summary_All$Objectives$name_objective)[i]),
...
Penser à `@export` pour toutes les fonctions et `@import` pour spider et recov..
Penser à `@data`
PEnser à `@example` pour toutes les fonctions

## 7. Git Package New
## 8. Shiny
### 8.1. Importer données + graphes hist
### 8.2. Project_all
### 8.3. Graphes "all"
### 8.4. Mettre le Shiny sur internet (git?)
